<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=180&section=header&text=Abdul%20Samad&fontSize=50&fontColor=fff&animation=twinkling&fontAlignY=38&desc=Web%20Developer%20%7C%20Automation%20Builder%20%7C%20AI%20Enthusiast&descAlignY=60&descAlign=50" />
</div>

<div align="center">
  <a href="mailto:abdulsamadx.ai@gmail.com"><img src="https://img.shields.io/badge/-Email-7C3AED?style=for-the-badge&logo=gmail&logoColor=white" alt="Email"/></a>
  <a href="https://github.com/abdulsamadxai"><img src="https://img.shields.io/badge/-GitHub-7C3AED?style=for-the-badge&logo=github&logoColor=white" alt="GitHub"/></a>
  <a href="https://shorturl.at/JR0I3"><img src="https://img.shields.io/badge/-Portfolio-7C3AED?style=for-the-badge&logo=firefox&logoColor=white" alt="Portfolio"/></a>
</div>

<br/>

## 👨‍💻 About Me

AI Engineering Intern at **Nayatel** · Studying **Bachelors in Artificial Intelligence** at COMSATS University Islamabad · Building at the intersection of web development, automation, and AI.

```javascript
const abdulSamad = {
    location:   "Islamabad, Pakistan 🇵🇰",
    education:  "BSc Artificial Intelligence @ COMSATS University Islamabad",
    work:       "AI Engineering Intern @ Nayatel",
    portfolio:  "https://shorturl.at/JR0I3",
    languages: {
        highLevel:    ["Python", "JavaScript", "SQL"],
        averageLevel: ["Java", "C++"],
        baseLevel:    ["TypeScript", "Bash"]
    },
    programming: {
        frontend:   ["React.js", "HTML", "CSS"],
        backend:    ["Node.js", "Express.js", "REST APIs"],
        databases:  ["MySQL", "JDBC"],
        automation: ["n8n", "LangChain", "Webhooks", "API Integration"],
        aiTools:    ["LangChain", "Groq", "ChromaDB", "RAG Pipelines"],
        tools:      ["Git", "GitHub", "VS Code"],
    },
    currentlyLearning: ["AI Engineering", "LangChain", "RAG", "n8n Advanced Automations"],
    funFact: "I build automations so I can spend more time building things 🤖",
};
```

<br/>

## 💼 Experience

| Role | Company | Period |
|:---|:---|:---:|
| **AI Engineering Intern** | Nayatel | 2026 – Present |
| **Web Developer** | Techvent Global | Jan 2026 – Present |
| **AI Automation Intern** | KHAWAJA Holdings | Nov 2025 – Dec 2025 |
| **Data Entry Operator** | Mayfair | Nov 2024 – Dec 2024 |

<br/>

## 🛠️ Tech Stack

<div align="center">
  <img src="https://skillicons.dev/icons?i=python,js,java,cpp,react,nodejs,express,mysql,git,github,html,css,vscode,langchain&theme=dark&perline=8" />
</div>


<br/>

## 🎓 Education

<div align="center">

| Degree | Institution | Period |
|:---|:---|:---:|
| **Bachelors in Artificial Intelligence** | COMSATS University Islamabad | Sep 2023 – Feb 2028 |

</div>

<br/>

## 🏆 Certifications

<div align="center">

| Certificate | Issuer | Year |
|:---|:---|:---:|
| **AI for Everyone** | DeepLearning.ai | 2025 |
| **Generative AI Fundamentals** | Microsoft | 2025 |
| **Python Crash Course** | Google | 2024 |

</div>

<br/>

## 🌐 Languages

<div align="center">

![Urdu](https://img.shields.io/badge/Urdu-Native-7C3AED?style=for-the-badge&logoColor=white)
![English](https://img.shields.io/badge/English-Fluent-7C3AED?style=for-the-badge&logoColor=white)
![Arabic](https://img.shields.io/badge/Arabic-Intermediate-7C3AED?style=for-the-badge&logoColor=white)
![Punjabi](https://img.shields.io/badge/Punjabi-Native-7C3AED?style=for-the-badge&logoColor=white)

</div>

<br/>

## 📊 GitHub Stats

<div align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=abdulsamadxai&layout=compact&theme=tokyonight&hide_border=true"/>
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api?username=abdulsamadxai&show_icons=true&theme=tokyonight&hide_border=true"/>
</div>

<br/>

<div align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=120&section=footer" />
</div>
